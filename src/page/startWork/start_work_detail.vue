<template>
  ...
  <div class="con_list">
    <el-popover v-model="sw_popover1" ref="check_fail_reason_popover_1" placement="left" width="400" trigger="click">
      <div class="popover-header">
        <span>请填写理由</span>
      </div>
      <div class="popover-content">
        <!--<div style="border:1px solid #d1dbe5;padding:10px;margin:5px;"></div>-->
        <el-checkbox-group v-model="sw_reasonList1" >
          <ul class="dialogUl">
        ...
        <p class="reason-show-p" style="margin:0;">
          <span class="fl mr5" style="line-height:30px;font-size:14px;">其他原因</span><el-input class="fl" style="width:50%;" name="sw_otherReason1" v-model="sw_otherReason1" size="small" label="" placeholder="请输入原因"></el-input>
        </p>
        <p v-show="errorTipShow1" class="reason-show-p" style="margin:0;color:red">请填写不通过理由</p>
      </div>
      <div class="popover-footer">
        <el-button size="small" @click="sw_saveReason(1)">提交</el-button>
      </div>
    </el-popover>
    ...
    <el-popover v-model="popover2" ref="check_fail_reason_popover_2" placement="left" width="400" trigger="click">
      ...
      <div class="popover-content">
        ...
        <p class="reason-show-p" style="margin:0;">
          <span class="fl mr5" style="line-height:30px;font-size:14px;">其他原因</span><el-input class="fl" style="width:50%;" name="sw_otherReason1" v-model="otherReason2" size="small" label="" placeholder="请输入原因"></el-input>
        </p>
        <p v-show="errorTipShow2" class="reason-show-p" style="margin:0;color:red">请填写不通过理由</p>
      </div>
      <div class="popover-footer">
        <el-button size="small" @click="sw_saveReason(2)">提交</el-button>
      </div>
    </el-popover>
    ...
    <el-row class="s-f-back" style="line-height:36px;">
      <el-col :span="24" style="text-align:right;">
        <el-button v-if="sw_showCheckLog==1" @click="requestCheckLog" type="info">查看审批轨迹</el-button>
        <el-button v-else @click="changeShowCheckLog" type="info">收起审批轨迹</el-button>
      </el-col>
    </el-row>
    <el-row v-bind:class="['s-f-back','mt10',sw_showCheckLog == 1 ? 'show-check-log': 'hide-check-log']" style="line-height:36px;">
      <el-col :span="24" >
        <el-table :data="checkLogs"  ref="singleTable" border highlight-current-row style="width: 100%">
  ...
  export default {
    data() {
      return {
        ...
        Final:'',
        sw_popover1:false,
        sw_reasonList1:[], //企业审核失败原因
        sw_otherReason1:'',//企业审核失败其他原因
        errorTipShow1:false,
        remark1:'',
        popover2:false,
...
//      ElCol,
    ...
    methods : {
      changeShowCheckLog () {
        if(this.sw_showCheckLog==1){
          this.sw_showCheckLog=2
        }else{
          this.sw_showCheckLog=1
        }
      },
      /**
       *请求审核日志详情
      ...
      requestCheckLogDetail  (auditId) {
        if(auditId){
          ...
          Api.sw_show_audit_log_detail(param)
            .then(res => {
              if(res.status==1){
      ...
      commitCheckData () {
        if(this.type1 && this.type2){
          ...
          Api.sw_check_info_commit(param)
            .then(res => {
              if(res.status==1){
                this.$message({
                  message: '审核成功',
                  ...
                  onClose: function (){
                    //此处回调
                    this.$router.push("/startwork/startwork_1/");
                  }.bind(this)
                });
              }
            }).catch(err =>{
      ...
      sw_saveReason (p) {
        if(p==1){
          if (this.sw_reasonList1.length==0 && this.sw_otherReason1==''){
            this.errorTipShow1=true;
            return;
          }else {
            this.sw_popover1=false;
            this.type1=2;
      ...
      formatterRemark (p) {
        if(p==1){
          var num=1;
          this.remark1="企业信息:"
          for(let i=0;i<this.sw_reasonList1.length;i++){
            this.remark1+=num+"、"+this.sw_reasonList1[i]+"  ";
            num++;
          }
          if(this.sw_otherReason1){
            this.remark1+=num+"、"+this.sw_otherReason1;
          }
        }else if(p==2){
          var num=1;
          this.remark2="企业信息:"
